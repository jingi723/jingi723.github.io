...
image:
  path: /assets/img/posts/edge-dev-log/ingestion-pipeline.drawio.png
  alt: edge 금융 데이터 Ingestion Pipeline 아키텍처
---
...
- **SW마에스트로 17기**
- 역할: 백엔드 · 데이터 파이프라인 개발 (2026.07~)

이 글은 edge에서 내가 맡은 **데이터 수집 파이프라인**을 "이력서 3줄"로 먼저 압축하고,
각 줄을 실제 설계 단위로 풀어 기록하는 개발 일지다.
[kth4778 님의 soma17 dev-log](https://kth4778.github.io/posts/soma17-dev-log/)를 모토로 삼아,
목표를 먼저 문장으로 못 박고 진행하면서 채워 나가는 방식을 택했다.
...
![edge Ingestion Pipeline 아키텍처: External Source → Collector → Kafka Topic → Crawling/Parsing Worker → Analysis Request Worker → Analysis Result DB, 실패 메시지는 DLQ와 Replay Worker로 재처리](/assets/img/posts/edge-dev-log/ingestion-pipeline.drawio.png)
_수집 → Kafka → worker 체인. 반복 실패 메시지는 DLQ로 보내고 replay worker로 재투입한다._
...
![Outbox 패턴 흐름: Collector Server가 news_raw와 outbox_events를 하나의 DB Transaction으로 저장하고, Debezium CDC가 outbox_events 변경을 캡처해 Kafka Topic으로 publish, Downstream Worker가 consume](/assets/img/posts/edge-dev-log/outbox-cdc.drawio.png)
_`news_raw`와 `outbox_events`를 한 트랜잭션으로 저장하고, outbox 변경만 CDC로 흘려보낸다._
...
![Kafka Reliability Lab: Consumer 처리 순서(① 메시지 처리 → ② DB 저장 → ③ manual offset commit)에서 장애 A(② 이후 ③ 이전 종료 시 중복)와 장애 B(③ 이후 ② 실패 시 유실)를 표시하고, poison message는 DLQ → Replay Worker로 흐름](/assets/img/posts/edge-dev-log/reliability-lab.drawio.png)
_offset commit을 ② DB 저장 앞/뒤 어디에 두느냐에 따라 중복(장애 A) 또는 유실(장애 B)이 갈린다._
